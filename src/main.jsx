...
const globeArcs = [[0, 2], [0, 5], [2, 3], [3, 5], [0, 4], [1, 5]];
...
function RegionGlobe() {
  ...
  useEffect(() => {
    ...
    const project = (lat, lng, size, rot) => {
      const phi = (lat * Math.PI) / 180;
      const theta = ((lng * Math.PI) / 180) + rot;
    ...
    const draw = () => {
      ...
      const glow = ctx.createRadialGradient(cx * 0.72, cy * 0.45, r * 0.1, cx, cy, r * 1.22);
      glow.addColorStop(0, 'rgba(95, 118, 180, 0.95)');
      glow.addColorStop(0.42, 'rgba(43, 55, 92, 0.94)');
      glow.addColorStop(1, 'rgba(22, 25, 35, 0.18)');
      ctx.fillStyle = glow;
      ctx.beginPath();
      ...
      for (let i = -60; i <= 60; i += 30) {
        ...
        const h = Math.cos((i * Math.PI) / 180) * r;
        ctx.beginPath();
        ctx.ellipse(cx, y, h, h * 0.22, 0, 0, Math.PI * 2);
        ctx.stroke();
      }
      for (let i = 0; i < 12; i += 1) {
        ctx.beginPath();
        ctx.ellipse(cx, cy, r * 0.24, r, (i * Math.PI) / 12 + rotation, 0, Math.PI * 2);
        ctx.stroke();
      }

      const points = globeCities.map((city) => project(city.lat, city.lng, size, rotation));
      for (const [from, to] of globeArcs) {
        const a = points[from];
        const b = points[to];
        if (!a.visible && !b.visible) continue;
        const alpha = Math.max(0.18, Math.min(0.82, (a.z + b.z + 2) / 4));
        ctx.strokeStyle = `rgba(244,122,50,${alpha})`;
        ctx.lineWidth = 1.6;
        ctx.beginPath();
        ctx.moveTo(a.x, a.y);
        const mx = (a.x + b.x) / 2;
      ...
      for (const p of points) {
        ...
        ctx.fillStyle = '#ff6631';
        ctx.beginPath();
        ctx.arc(p.x, p.y, 4.2, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = 'rgba(255, 102, 49, 0.22)';
        ctx.lineWidth = 8;
        ctx.stroke();
      }
      ctx.restore();
